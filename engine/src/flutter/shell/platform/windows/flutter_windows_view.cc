...
void FlutterWindowsView::SendWindowMetrics(size_t width,
  ...
  if (IsSizedToContent()) {
    ...
    event.min_width = static_cast<size_t>(min_size.width());
    event.min_height = static_cast<size_t>(min_size.height());
    event.max_width = static_cast<size_t>(max_size.width());
    event.max_height = static_cast<size_t>(max_size.height());
  } else {
    event.width = width;
    event.height = height;
  }
